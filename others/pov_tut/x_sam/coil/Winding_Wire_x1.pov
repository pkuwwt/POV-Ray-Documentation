// POV-Ray 3.7 Scene File "Winding_Wire_x1.pov"
...
#case (0)
  #declare Camera_Location = < 0.00, 0.50, -2.50> ;  // front view
...
#break
#case (2)
  #declare Camera_Location = < 3.0, 0.5 , 0.0> ;  // right side view
  #declare Camera_Look_At  = < 0.0, 0.5,  0.0> ;
  #declare Camera_Angle    =  40 ;
#break
#case (3)
  #declare Camera_Location = < 0.00, 3.00,  0+0.000> ;  // top view
...
//light_source{< 500,800,-2000> color rgb<1,1,1>*0.9}                // sun
...
union{
 ...
 text   { ttf Font_Name,  "y",  0.15,  0  texture{Tex_Dark}
          rotate<20,0,0> scale 0.65 translate <-0.65,AxisLenY+0.00,-0.00>  rotate<0,-35,0> no_shadow}
#end // of #if
#if (AxisLenZ != 0)
 object { Axis_(AxisLenZ, Tex_Dark, Tex_Light)   rotate<90,0,  0>}// z-Axis
 text   { ttf Font_Name,  "z",  0.15,  0  texture{Tex_Dark}
          rotate<20,-35,0> scale 0.65 translate <-0.75,0.2,AxisLenZ+0.10> no_shadow}
#end // of #if
} // end of union
...
object{ AxisXYZ( 3.70, 2*1.90, 2*4.1, Texture_A_Dark, Texture_A_Light) scale 0.2 no_shadow }
...
plane { <0,1,0>, 0    // plane with layered textures
        texture { pigment{color White*0.05}
...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
#declare Wire_Texture = 
         texture {  pigment{ color rgb< 0.85, 0.37, 0.15>*0.59 }
                    normal { bumps 0.1 scale 0.15}
                    finish { phong 1 specular 0.0 
                             reflection{ 0.55 metallic 0.5 } }
                 } // end of texture
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
#include "math.inc"
...
#local WD   = 0.19; // vertical wire distance: min 2*WR  (pitch)
#local BR   = 0.50; // column base radius (to wire center)
#local Rmaj = 0.30; // corner radius major of wire 
#local Revolutions = 4.5 ;// 0.25, 0.5, 0.75, 1, .... 
...
union{ 
     ...
     object  { W_Corner  rotate<-Inner_Angle,0,0>  translate< Len_X,0,0> }  
     cylinder{ <0,0,0>,< Len_X,0,0>, WR 
...
#if( Kernel_On = 1 ) //------------------------------------------------------
#local KH = (Revolutions+0.0)*WD+1*WR; // kernel height

#local KH = KH/4; // kernel height
#local KR = BR-WR; // kernel height
#local Rm = Rmaj-WR; // kernel height
merge{
 box{<-KR+Rm,0,-KR>,<KR-Rm,KH+D,KR> }
 box{<-KR,0,-KR+Rm>,<KR,KH-D,KR-Rm> }
 cylinder{ <0,0,0>,<0,KH,0>,Rm translate< KR-Rm,0, KR-Rm> }
 cylinder{ <0,0,0>,<0,KH,0>,Rm translate< KR-Rm,0,-KR+Rm> }
 cylinder{ <0,0,0>,<0,KH,0>,Rm translate<-KR+Rm,0, KR-Rm> }
 cylinder{ <0,0,0>,<0,KH,0>,Rm translate<-KR+Rm,0,-KR+Rm> }
 texture{ pigment{ color rgbt< 1, 1, 1, 0.0>*1.0 } //  color White
