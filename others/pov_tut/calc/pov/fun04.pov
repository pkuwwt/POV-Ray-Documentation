// POV-Ray 3.6 / 3.7 Scene File "fun04.pov"
...
#declare Camera_0 = camera { perspective
                             location  < 0.00, 2.95,-40.0>
                             look_at   < 0.00, 2.95, 0.00>
                             angle 15.7
...
union{
 ...
 text   { ttf "arial.ttf",  "5",  0.15,  0  texture{Tex_Dark}
          scale <0.6,0.35,0.6> translate <-0.35,5.1,-0.02>}
 text   { ttf "arial.ttf",  "-5",  0.15,  0  texture{Tex_Dark}
...
union{
 ...
 // Zeichnen der "Punkte"  ( X / f(X) ):
                       ...
                       //       Color - Farbe                  //< X, f(X)         ,0>}
  sphere{ <0,0,0>,0.045 pigment{ color rgb<0.9,0.0,0.3>  }  translate< X, sin(X *pi),0>}

  sphere{ <0,0,0>,0.045 pigment{ color rgb<0.8*abs(sin(X*pi)),0.0,0.8*abs(cos(X*pi))>  }  translate< X,2*abs( sin(X*pi) ) +1,0>}

  sphere{ <0,0,0>,0.045 pigment{ color rgb<0.4,0.1,0.7>  }  translate< X, abs(5* mod( sin(X/2 *pi), 0.25) +5),0>}

 // next step:
 #declare X = X + 0.001;  // next Nr  <<<-- increase this value if you have not enough RAM 
