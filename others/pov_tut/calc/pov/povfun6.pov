// POV-Ray 3.6 / 3.7 Scene File "povfun6.pov"
...
union{
 ...
 // Zeichnen der "Punkte"  ( X / f(X) ):
                       ...
                       //       Color - Farbe                  //< X, f(X)           ,0>}
  sphere{ <0,0,0>,0.025 pigment{ color rgb<1,0.65,0> }  translate< X, (2*X*X+4*X)/((X+1)*(X+1)) ,0>}

  sphere{ <0,0,0>,0.025 pigment{ color rgb<0.5,1,0>  }  translate< X, 3/2*X*X*X-4*X*X+4*X ,0>}
 ...
 #declare X   = 0.01; // start X    Attention: x != X
 #declare EndX = 1; // end   X    
 #while ( X < EndX+0.025 ) 
  cylinder { <X,0,0>,<X, (2*X*X+4*X)/((X+1)*(X+1)) ,0>,0.025 
             texture { pigment { color  rgb<0.75, 0.0, 0.2>}
                       finish  { ambient 0.15 diffuse 0.8 phong 1}
                     } // end of texture
           } // end of cylinder

 #declare X = X + 0.05;  // next Nr
 #end // --------------- end of loop 

cylinder { <0,-6,0>,<0,6,0>,0.025 
           texture { pigment { color  rgb<1, 0.0, 0.1>}
                     finish  { ambient 0.15 diffuse 0.8 phong 1}
                   } // end of texture
           translate< -1,0,0>
         } // end of cylinder

cylinder { <-6,0,0>,<6,0,0>,0.025 
           texture { pigment { color  rgb<1, 0.0, 0.1>}
                     finish  { ambient 0.15 diffuse 0.8 phong 1}
                   } // end of texture
           translate< 0,2,0>
